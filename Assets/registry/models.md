...
| ArmySoldier (RocketMan enemy rig) | **11927692797** | jungle | **Creator Store, FREE, third-party — SCANNED: 0 scripts.** "US Army Soldier" by HK_BaconBest827943. Full **R15** rig, 15 Motor6D, Humanoid, standard R15 part names, `RightGripAttachment` on `RightHand`. Jungle Job #119: becomes the `RocketMan` on the deep camp's `RangerTower`. Filed at `ServerStorage.AssetLibrary.Enemies.ArmySoldier` (**place file only — AssetLibrary is not in git**). `scale nil` (5.77 studs is already right), `yawOffset 0` (HumanoidRootPart LookVector measured at (0,0,-1) = our forward), `eyes = false` (he is a person). ⚠️ **STRIPPED AT IMPORT:** arrives 122 parts, of which a **37-part `Mk18 EoTech` carbine** is welded to `RightHand` (with a `ProjectorSight` SurfaceGui and 9 gun-mechanism Sounds) — deleted, because he carries the Bazooka. Leaves **85 parts**: 16 body + a 39-part helmet (`Head > Model`) + a 30-part plate carrier (`UpperTorso > Model`). Helmet and vest KEPT — they are what makes him read as army. ⚠️ **Its inner model ships named `" "` (a single space)** — renamed `ArmySoldier`, wrapper dropped, because `EnemyRig`'s library lookup is by NAME and is exactly two levels (`AssetLibrary/<group>/<model>`). ⚠️ **An `Animator` was ADDED under its Humanoid.** It ships a Humanoid with NO Animator, which sent `EnemyRig` down its "create an AnimationController beside the live Humanoid" branch — two drivers, plays nothing. With the Animator present it takes the Bandit's proven reuse path. ⚠️ **Its helmet and vest parts are held on by `Anchored` alone, with ZERO welds** — so unanchoring the rig dropped all 69 of them off the model and the engine destroyed them past `FallenPartsDestroyHeight`. Live rig measured **17 parts against the library's 85** before the fix. `EnemyRig` now welds loose nested accessory parts to their host body part before unanchoring (a `Weld`, not a `WeldConstraint` — Job #084's finding). ⚠️ **`UpperTorso.Model.Chest.Border` uses mesh 4785764674, which fails to fetch** ("could not fetch" in the log) — one small vest trim piece does not render. Cosmetic; logged as a finding. |
